...
### Alpha diversity by types of pond
```{r}
alpha_div %>%
...
##Negative Binomial
library(MASS)
Modelq0GLMMNB <- glm.nb(richness ~ type, data = alpha_div_meta,trace=TRUE)#USE THIS!
summary(Modelq0GLMMNB)
emmeans(Modelq0GLMMNB, pairwise ~ type)
detach("package:MASS", unload = TRUE)
```

```{r mixed_neut_type, comment="", echo=FALSE, message=FALSE, warning=FALSE}
Modelq1_Loca <- lm(formula = neutral ~ type, data = alpha_div_meta) 
summary(Modelq1_Loca)
emmeans(Modelq1_Loca, pairwise ~ type)
```

```{r mixed_phylo_type, comment="", echo=FALSE, message=FALSE, warning=FALSE}
Modelq1p_Loca <- lm(formula = phylogenetic ~ type, data = alpha_div_meta) 
summary(Modelq1p_Loca)
emmeans(Modelq1p_Loca, pairwise ~ type)
```

```{r mixed_funct_type, comment="", echo=FALSE, message=FALSE, warning=FALSE}
Modelq1F_Loca <- lm(formula = functional ~ type, data = alpha_div_meta) 
summary(Modelq1F_Loca)
emmeans(Modelq1F_Loca, pairwise ~ type)
```
